 ...
 * empty. The slots are referred to by their "code", which is a letter. These
 * codes are "A" for the slot at index 0, "B" for the slot at index 1, "C" for
 * the slot at index 2, and "D" for the slot at index 3.
 */
public class VendingMachine {
    ...
    /**
     * Gets the slot index given the code for that slot. Specifically, the codes
     * are "A" for slot index 0, "B" for slot index 1, "C" for slot index 2 and
     * "D" for slot index 3.
     * 
     * @param code
     *            The code for the slot.
	...
	if (code.equals(A_CODE)) {
	    ...
	    throw new VendingMachineException(
		    VendingMachine.INVALID_CODE_MESSAGE);
	}
    }
     ...
     * 
     * Precondition: The slot specified by the code must be empty Postcondition:
     * The item is now at the slot specified by the code
     * 
     * @param item
     ...
     * 
     * @param code
     *            The code for the the slot in the vending machine eg. "A"
     * @return The item occupying the slot with the given code
     * @throws VendingMachineException
     *             if the code is invalid
     */
    protected VendingMachineItem getItem(String code)
	    throws VendingMachineException {
	int slotIndex = getSlotIndex(code);
    ...
    /**
     * Removes an item from the vending machine given its code. Postcondition:
     * If the code slot is not empty, the item in that slot is removed.
     * 
     * @param code
     *            The code eg. "A" of the item in the vending machine
     * @return The item occupying the slot with the given code.
     * @throws VendingMachineException
     *             If the slot at the specified code is empty and if the code is
     *             invalid
     */
    public VendingMachineItem removeItem(String code)
	    throws VendingMachineException {
	int slotIndex = getSlotIndex(code);
	...
	if (amount < 0)
	    throw new VendingMachineException(
		    VendingMachine.INVALID_AMOUNT_MESSAGE);
	this.balance += amount;
    }
     ...
     * vending machine. Precondition: balance >= 0 Postcondition: The amount of
     * the item is subtracted from the balance
     * 
     * @param code
     *            The code for the item from the vending machine
     ...
     *         to make the purchase. Also returns false if the code is for an
     *         empty slot.
     */
